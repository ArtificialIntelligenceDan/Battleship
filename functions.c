...
char carrierIcon = 'C';
char battleshipIcon = 'B';
char cruiserIcon = 'R';
char submarineIcon = 'S';
char destroyerIcon = 'D';
        ...
        for (x = 0; x < 10; x++){
            ...
            for (row = 0; row < 3; row++){
                ...
                for (col = 0; col < 5; col++){
                    if (row == 1 && col == 2){
                        if (grid == PLAYER){
                            if (shipGrid[x][y][grid] == CARRIER){
                                displayGrid[2*y + row][4*x + col][grid] = carrierIcon;
                            }
                            else if (shipGrid[x][y][grid] == BATTLESHIP){
                                displayGrid[2*y + row][4*x + col][grid] = battleshipIcon;
                            }
                            else if (shipGrid[x][y][grid] == CRUISER){
                                displayGrid[2*y + row][4*x + col][grid] = cruiserIcon;
                            }
                            else if (shipGrid[x][y][grid] == SUBMARINE){
                                displayGrid[2*y + row][4*x + col][grid] = submarineIcon;
                            }
                            else if (shipGrid[x][y][grid] == DESTROYER){
                                displayGrid[2*y + row][4*x + col][grid] = destroyerIcon;
                            }
    ...
    for (row = 0; row < 21; row++){ // Print grid spaces
        for (grid = 0; grid < 2; grid++){
            ...
            for (col = 0; col < 41; col++){
                int x = col/4;
                int y = row/2;
                if (grid == PLAYER) // If player's grid
                    if (displayGrid[row][col][grid] == carrierIcon
                        || displayGrid[row][col][grid] == battleshipIcon
                        || displayGrid[row][col][grid] == cruiserIcon
                        || displayGrid[row][col][grid] == submarineIcon
                        || displayGrid[row][col][grid] == destroyerIcon){
                ...
                else if (grid == OPPONENT){ // If opponent's grid
